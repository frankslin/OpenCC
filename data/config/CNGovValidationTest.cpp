 ...
 *
 * Validation test for CN Government Standard (t2cngov) configs.
 */

#ifndef BAZEL
// This test is Bazel-only; CMake builds should skip compiling it.
static_assert(false, "CNGovValidationTest is only supported under Bazel");
#else
...
class CNGovValidationTest : public ::testing::Test {
protected:
  void SetUp() override {
#ifdef BAZEL
    runfiles_.reset(Runfiles::CreateForTest());
    ASSERT_NE(nullptr, runfiles_);
    testcasesPath_ = runfiles_->Rlocation("_main/test/testcases/cngov_testcases.json");
...
TEST_F(CNGovValidationTest, ConvertExpectedOutputs) {
