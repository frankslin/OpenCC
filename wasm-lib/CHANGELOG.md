...
- **Stale bundled dictionaries**: Regenerated the main `.ocd2` assets, which had drifted behind the checked-in text dictionaries: `STPhrases`, `STPhrases_GeneratedFromRegionalPhrases`, `TWPhrases`, `TWPhrasesRev`, `TWVariants`, `TWVariantsRev`, `TWVariantsRevPhrases`, `HKPhrases` and `HKPhrasesRev`. The most visible symptom was `s2t` converting `范氏` to `範氏` instead of keeping the surname. All bundled dictionaries are now byte-identical to the ones built from `data/dictionary` and shipped in `opencc` 1.4.2.
- **Stale bundled test cases**: Refreshed `test/testcases.json` from `test/testcases/testcases.json`; the bundled copy was missing 21 cases, including the `范氏` regression that would have caught the stale dictionaries.
- **Benchmark**: Added `npm run benchmark:compare` (`scripts/benchmark-compare.mjs`), which measures `opencc`, `opencc-wasm` and `opencc-js` on identical workloads (startup, 1.8 MB long text, short-string latency, RSS) in separate processes and reports output hashes so any behavioural difference is visible alongside the timings. Tofu-risk dictionaries are excluded by default so all three run the same dictionary set (`opencc-js` has no switch for them); pass `--include-tofu` for library defaults.
